...
Adafruit_MQTT_Publish disco = Adafruit_MQTT_Publish(&mqtt, AIO_USERNAME "/feeds/DiscoSkull");
...
void loop() {
  ...
  if(currentMillis - startMillis >= 15000) { //test whether the maximum fill period has elapsed
    //Send a MQTT signal that the other lights look for to turn on.
    disco.publish(1);
...
void wifiSetup() {
  ...
  ArduinoOTA.setHostname("DiscoSkullMaster");
  ArduinoOTA.onStart([]() {
    String type;
